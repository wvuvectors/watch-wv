...
df_thresholds <- as.data.frame(read.table(DB_THRESHOLDS, sep="\t", header=TRUE, check.names=FALSE))
...
write.table(df_pcr_wvu_out, file = paste(DB_BASE, "/watchdb.result.txt", sep=""), sep = "\t", quote = FALSE, row.names = FALSE)
write.table(df_pcr_mu_out, file = paste(DB_BASE, "/mu.result.txt", sep=""), sep = "\t", quote = FALSE, row.names = FALSE)
