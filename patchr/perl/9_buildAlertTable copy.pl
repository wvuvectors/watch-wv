...
my $WINDOW = 3;	# Number of weeks to use in trend calculations.
...
# Arrange asset ids for each location, county, and target in chronological order to facilitate 
# construction of the arrays in the recents hash (next step).
# 
my %ordered_uids = ();
...
# Build a hash keyed on region (either county or location) and target of the WINDOW most 
# recent weeks of samples, in chronological order.
# Multiple samples from the same week are averaged for simplicity.
# 
my %recents = ();
my %local_state_rollup = ();

foreach my $county (keys %results) {
	$recents{"$county"} = {};
	my %local_county_rollup = ();
	foreach my $locid (keys %{$results{"$county"}}) {
		$recents{"$locid"} = {};
		foreach my $targ (keys %{$results{"$county"}->{"$locid"}}) {
			$recents{"$locid"}->{"$targ"} = [];
			$recents{"$county"}->{"$targ"} = [] unless defined $recents{"$county"}->{"$targ"};
			$local_county_rollup{"$targ"} = {} unless defined $local_county_rollup{"$targ"};
			$local_state_rollup{"$targ"} = {} unless defined $local_state_rollup{"$targ"};
			my $this_win = $WINDOW;
			$this_win = scalar @{$ordered_uids{"$county"}->{"$locid"}->{"$targ"}}-1 if $this_win > scalar @{$ordered_uids{"$county"}->{"$locid"}->{"$targ"}}-1;
			my $week_count = -1;
			my $sample_count = 0;
			my $current_week = 0;
			my %week_vals = ();
			while ($week_count <= $this_win) {
				if ($sample_count >= scalar @{$ordered_uids{"$county"}->{"$locid"}->{"$targ"}}) {
					warn "sample count ($sample_count) too big for $locid, $targ!\n";
					last;
				}
				my $uid = $ordered_uids{"$county"}->{"$locid"}->{"$targ"}->[$sample_count];
				$sample_count++;
				my $this_week = $results{"$county"}->{"$locid"}->{"$targ"}->{"$uid"}->{"collection_week"};
				if ($this_week != $current_week) {
					# new week so take average of current values and push to array
					if (scalar keys %week_vals > 0) {
						my $mean = calcSimpleMean($week_vals{$current_week});
						%week_vals = ();
					}
					$current_week = $this_week;
					$week_vals{$current_week} = [];
					$week_count++;
				}
				my $val = $results{"$county"}->{"$locid"}->{"$targ"}->{"$uid"}->{"target_copies_fn_per_cap"};
				if ("$val" eq "NA") {
					# Use only time-normalized data for upstream samples.
					$val = $results{"$county"}->{"$locid"}->{"$targ"}->{"$uid"}->{"target_copies_per_ld"};
				} else {
					# Normalize to per person.
					$val = $val / $results{"$county"}->{"$locid"}->{"$targ"}->{"$uid"}->{"target_per_capita_basis"};
				}
				push @{$week_vals{$current_week}}, $val;
			}
			# Got all the values for this location/target combo in chrono order in @week_vals.
			foreach my $week (keys %week_vals) {
				# Add each value directly to the recents hash for the location.
				push @{$recents{"$locid"}->{"$targ"}}, $week_vals{$week};
				# Also add it to the local loop's county hash under the correct key.
				# These will be rolled up after all location/target combos for this county are finished.
				$local_county_rollup{"$targ"}->{$week} = [] unless defined $local_county_rollup{"$targ"}->{$week};
				push @{$local_county_rollup{"$targ"}->{$week}}, $week_vals{$week};
				# Also add it to the local state hash under the correct key.
				# These will be rolled up after all county/location/target combos are finished.
				$local_state_rollup{"$targ"}->{$week} = [] unless defined $local_state_rollup{"$targ"}->{$week};
				push @{$local_state_rollup{"$targ"}->{$week}}, $week_vals{$week};
			}
		} # End of loop over targets in this location.
	} # End of loop over locations in this county.
	# Roll up local county values.
	foreach my $targ (keys %local_county_rollup) {
		foreach my $week (keys %{$local_county_rollup{"$targ"}}) {
			my $mean = calcSimpleMean($local_county_rollup{"$targ"}->{$week});
			push @{$recents{"$county"}->{"$targ"}}, $mean;
		}
	}
} # End of loop over counties.

# Roll up local state values.
foreach my $targ (keys %local_state_rollup) {
	foreach my $week (keys %{$local_state_rollup{"$targ"}}) {
		my $mean = calcSimpleMean($local_state_rollup{"$targ"}->{$week});
		push @{$recents{"WV"}->{"$targ"}}, $mean;
	}
}

print Dumper(\%recents);
die;
...
foreach my $county (keys %calc) {
	...
	foreach my $locid (keys %{$calc{"$county"}}) {
		...
		foreach my $targ (keys %{$calc{"$county"}->{"$locid"}}) {
			...
			$loc_rollup{"$locid"}->{"$targ"} = {};
			$county_rollup{"$county"}->{"$targ"} = {} unless defined $county_rollup{"$county"}->{"$targ"};
