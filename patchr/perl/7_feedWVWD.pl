...
$usage   .=  "Build feed table for WVWD using WaTCHdb tables in DBDIR (data/latest/ by default) ";
...
#
# Read data from WaTCH-WVWD mapping table. Keyed by extref column header.
#
my %map = ();
...
foreach my $assay_id (keys %{$tables{"result"}}) {
	...
	#
	# Skip if pcr_target is not SARS-CoV-2.
	next unless "$resultRef->{target}" eq "SARS-CoV-2";
	#
	# Skip if pcr_gene_target is not N2:SARS.
	next unless "$resultRef->{target_genetic_locus}" eq "N2:SARS";
	#	
	...
	foreach my $header (@feed_headers) {
		...
		if ("$mapEntry{WaTCH_field}" eq "[empty]") {
			...
			my $lfield = "";
			$lfield = "$mapEntry{linked_field}" if defined $mapEntry{"linked_field"} and $mapEntry{"linked_field"} ne "";
...
sub lookupLinked {
	...
	} elsif ("$tname" eq "sample") {
		$linkedId = $tables{"result"}->{"$aid"}->{"sample_id"};
	} elsif ("$tname" eq "lab") {
		$table_cat = "resource";
		$linkedId = $tables{"result"}->{"$aid"}->{"lab_id"};
	}
...
sub calcExtref {
	...
	} elsif ("$extref_field" eq "category") {
		my $locid = $tables{"result"}->{"$aid"}->{"location_id"};
		$val = uc $resources{"location"}->{"$locid"}->{"location_category"};
		$val = "upstream" unless lc($val) eq "wwtp";
	} elsif ("$extref_field" eq "group") {
		my $locid = $tables{"result"}->{"$aid"}->{"location_id"};
		$val = uc $resources{"location"}->{"$locid"}->{"location_category"};
		$val = "Sewer Network" unless lc($val) eq "wwtp";
	} elsif ("$extref_field" eq "level") {
		my $locid = $tables{"result"}->{"$aid"}->{"location_id"};
		$val = uc $resources{"location"}->{"$locid"}->{"location_category"};
		$val = "Sewer Network" unless lc($val) eq "wwtp";
	} elsif ("$extref_field" eq "collection_scheme") {
		my $locid = $tables{"result"}->{"$aid"}->{"location_id"};
