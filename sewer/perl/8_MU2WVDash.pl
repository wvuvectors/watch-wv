...
$usage   .=  "Extracts the MU data from the csv on STDIN, converts to watchdb format, and writes to STDOUT.\n";
$usage   .=   "\n";
...
while (my $line = <>) {
	...
	if ($csvA->parse($line)) {
		...
		if ($linenum == 0) {
			...
			die "Unable to parse a lab_id column in $progname!\n" unless $labindx > -1;
...
#open(my $RFH, ">", "../dashboard/data/mu.result.UNTAGGED.txt") or die "Unable to open result file for writing: $!";

print join("\t", @result_hdr) . "\n";
