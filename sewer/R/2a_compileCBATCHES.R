...
batch_df <- data.frame(
  ...
  concentration_batch_machine = character(),
  concentration_batch_method = character(),
  concentration_batch_method_lot_id = character(),
  concentration_batch_input_ml = character(),
  concentration_batch_output_ml = character(),
  concentration_batch_date = character(),
  concentration_batch_run_by = character(),
  concentration_batch_id = character(),
  concentration_batch_comment = character()
)

# Holds data for the individual concentrations in this batch.
run_df <- data.frame(
...
# Loop over the concentration batch files in this update.
...
#	i = 1
  ...
  this_fpath <- paste0(UPDIR, "/BATCH_FILES/", this_fn, sep="")
	...
	batch_keys <- str_replace_all(batch_keys, "^", "concentration_batch_")
	batch_keys <- str_replace_all(batch_keys, "^concentration_batch_concentration_", "concentration_batch_")
	batch_keys <- str_replace_all(batch_keys, "^concentration_batch_batch_", "concentration_batch_")
	batch_keys <- str_replace_all(batch_keys, "^concentration_batch_type", "batch_type")
	...
	batchup_df$concentration_batch_date <- convertToDateTime(as.numeric(batchup_df$concentration_batch_date))
	...
	# Process the plate map.
	plate_map_df <- platef_in[["Transfer Plate Map"]]
	plate_map_df <- plate_map_df[c(2:9), c(2:13)] 
	colnames(plate_map_df) <- c(1:12)
	rownames(plate_map_df) <- LETTERS[1:8]
#	plate_map_df <- plate_map_df %>% select(where(~!all(is.na(.))))
...
if (nrow(run_df) > 0) {
	...
	batch_df <- batch_df %>% mutate(
		validation_key = case_when(
			is.na(concentration_batch_id) | concentration_batch_id == "" ~ -1, 
			is.na(concentration_batch_sample_input_vol_ml) | concentration_batch_sample_input_vol_ml == "" ~ -1, 
			is.na(concentration_batch_elution_output_vol_ul) | concentration_batch_elution_output_vol_ul == "" ~ -1, 
			is.na(concentration_batch_method) | concentration_batch_method == "" ~ -1, 
			.default = 1
		)
	)
