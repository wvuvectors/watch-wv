...
my ($runMetaF, $assayDataF) = ("run_metadata.csv", "assay_plate.csv");
...
die "FATAL: $progname requires a valid $runMetaF file.\n$usage\n" unless -f "$rundir/$runMetaF";
die "FATAL: $progname requires a valid $assayDataF file.\n$usage\n" unless -f "$rundir/$assayDataF";
...
my %TARGETS = ("Assay Target 1" => "Flu A",
							 "Assay Target 2" => "Flu B",
							 "Assay Control, Internal" => "Rpp30");
...
#			$key = "Control, Process (Type)" if "$key" eq "Assay Process Control";
		...
		if ("$asset_id" =~ /^PCR/) {
			...
			# Initialize this sample in the flu db hash.
			if (defined $run2data{"$asset_id"}) {
				print "$progname line 204.\n";
				print "Sample $asset_id already exists in the run! It will be overwritten.\n";
			} else {
				$run2data{"$asset_id"} = {};
				for (my $i=0; $i < scalar(@r_field_names); $i++) {
					$run2data{"$asset_id"}->{"$r_field_names[$i]"} = "";
				}
			}
			...
			for (my $i=1; $i < scalar(@values); $i++) {
				next if "$values[$i]" eq "";
				$run2data{"$asset_id"}->{"$r_field_names[$i]"} = "$values[$i]";
...
#
my %well2assay = ();
my %p_field_pos = ("Target" => -1, "Status" => -1, "Experiment" => -1, "DyeName(s)" => -1, "Conc(copies/uL)" => 6, "Accepted Droplets" => -1);
...
while (my $line = $csvP->getline($assayDataFH)) {
	...
	if ($count == 0) {
		...
		for (my $i=0; $i < scalar(@values); $i++) {
#			$values[$i] =~ s/µ/u/gi;
#			$values[$i] =~ s/FLUA/Flu A/gi;
#			$values[$i] =~ s/FLUB/Flu A/gi;
#			$values[$i] =~ s/RPP30/Rpp30/gi;
			$p_field_pos{"$values[$i]"} = $i if defined $p_field_pos{"$values[$i]"};
		}
#		print Dumper(\%p_field_pos);
#		die;
	} else {
		...
		#$well2assay{$well_id}->{"$target_name"}->{"Conc(copies/ul)"} = $cn_per_ul;
...
#
# merge data from current run - stored in %well2assay and keyed by well id - into the main run2data hash - keyed by asset id
...
foreach my $asset_id (keys %sample2well) {
	...
	# Transfer it over into the master flu2data hash, which is keyed by the sample/asset id
	#
	unless (defined $well2assay{$well_id}) {
		...
		print "# Flu db update ABORTED.\n";
		print "# $END.\n";
		die;
	}

	my %sample_data = %{$well2assay{$well_id}};
	$run2data{"$asset_id"}->{"Assay Droplet Quantification Method"} = $sample_data{"Status"};
	$run2data{"$asset_id"}->{"Assay Experiment Type"} = $sample_data{"Experiment"};
	$run2data{"$asset_id"}->{"Assay Accepted Droplets"} = $sample_data{"Accepted Droplets"};
	...
	foreach my $target_id (keys %TARGETS) {
		...
		if (defined $sample_data{"$target_name"}) {
			...
			my $assay_result = 0;
			my $rid = "1.1";
			$rid = "1.2" if defined $asset2data{"$asset_id"}->{"1.2"};
			$rid = "1.3" if defined $asset2data{"$asset_id"}->{"1.3"};
			$assay_result = calc_result("$asset_id", "$rid", $sample_data{"$target_name"}->{"Concentration (CN/Rxn)"}) unless $sample_data{"$target_name"}->{"Concentration (CN/Rxn)"} < 0;
			$run2data{"$asset_id"}->{"$target_id"} = "$target_name";
			$run2data{"$asset_id"}->{"$target_id Result (CN/L)"} = $assay_result;
			$run2data{"$asset_id"}->{"$target_id Concentration (CN/Rxn)"} = $sample_data{"$target_name"}->{"Concentration (CN/Rxn)"};
			...
			if (defined $ctl2data{"PCR NC"} and defined $ctl2data{"PCR NC"}->{"Assay Plate Location"} and defined $well2assay{$ctl2data{"PCR NC"}->{"Assay Plate Location"}}) {
				...
				$run2data{"$asset_id"}->{"$target_id PCR NC Result (CN/Rxn)"} = $nc_data{"$target_name"}->{"Concentration (CN/Rxn)"};
			} else {
				$run2data{"$asset_id"}->{"$target_id PCR NC Result (CN/Rxn)"} = "NA";
			}
			...
			if (defined $ctl2data{"PCR PC"} and defined $ctl2data{"PCR PC"}->{"Assay Plate Location"}  and defined $well2assay{$ctl2data{"PCR PC"}->{"Assay Plate Location"}}) {
				...
				$run2data{"$asset_id"}->{"$target_id PCR PC Result (CN/Rxn)"} = $pc_data{"$target_name"}->{"Concentration (CN/Rxn)"};
			} else {
				$run2data{"$asset_id"}->{"$target_id PCR PC Result (CN/Rxn)"} = "NA";
			}
...
#print Dumper(\%run2data);
#die;


#
# %flu2data now contains all the flu data
...
if ($dowrite == 0) {
	...
	foreach my $asset_id (keys %run2data) {
		my $fluset = $run2data{"$asset_id"};
		my $asset  = $asset2data{"$asset_id"}->{"1.1"};
		print $watchFH "$asset_id";
		foreach my $field (@flu_outfields) {
			next if "$field" eq "Sample ID";
			my $value = "NA";
			if (defined $fluset->{"$field"} and $fluset->{"$field"} ne "") {
				$value = "$fluset->{$field}";
			} elsif (defined $asset->{"$field"} and $asset->{"$field"} ne "") {
				$value = "$asset->{$field}";
			}
			print $watchFH "\t$value";
		}
...
print "INFO: WaTCHdb flu update complete.\n";
...
sub calc_result {
	...
	$calc_vals{"Assay Volume (mL)"} = $run2data{"$aid"}->{"Assay Volume (mL)"};
	$calc_vals{"Assay Volume (mL)"} = $asset2data{"$aid"}->{"$rid"}->{"Assay Volume (mL)"} unless defined $calc_vals{"Assay Volume (mL)"};
