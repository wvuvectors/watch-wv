...
#						absolutePanel(
#							top = 80, right = 60, width = "auto", 
#							fixed=TRUE, draggable = FALSE, 
#							height = "auto",
#							style="margins: 1px 0px 5px 0px; padding: 3px;",
#							actionBttn(inputId="embiggen_close_wwtp", label="Close", style="pill", size="xs", color="success")
#						)
					)
				)
			)
							...
							column(
								...
								div(
									...
									sliderTextInput(
										...
										#width = "90%",
										label = h6("Choose a date range (by week):"),
										choices = unique(as.Date(ymd(df_upstream$week_starting))),
										selected = c(as.Date(min(ymd(df_upstream$week_starting))), as.Date(max(ymd(df_upstream$week_starting)))),
			#							animate=animationOptions(interval = 3000, loop = FALSE),
										grid = FALSE
									)
								)
