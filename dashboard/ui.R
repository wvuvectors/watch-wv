...
#					div(downloadLink(outputId = "downloadData", label = NULL, class = "dlsmall"), "Download (csv)", style="display: inline-block; font-size: 9px; font-weight: 400;text-align: left;width: 25px;height: 25px;padding-top: 5px;")
				...
				absolutePanel(
					...
					tableHTML(
						df_alert_county %>% select(County, FLUA, FLUB, COVID, RSV), 
					...
					add_css_column(css = list("text-align", "center"), columns=names(df_alert_county[2:5])) %>% 
					add_css_table(css = list("background-color", "#ffffff")) %>% 
					add_css_row(css = list("background-color", "#f2f2f2"), rows = odd(1:length(df_alert_county$County)+1)) %>%
					add_css_row(css = list("background-color", "#e6f0ff"), rows = even(1:length(df_alert_county$County)+1))
				), # absolutePanel
